...
2. **策略评估（Policy Evaluation）**：
   ...
   - 更新规则：V_{k+1}(s) = Σ_a π(a|s) Σ_{s',r} P(s',r|s,a)[r + γV_k(s')]
...
这其实是贝尔曼方程的固定点迭代，当 $V_{k+1} = V_k$ 时收敛到 $V^\pi$。
...
- **策略评估收敛**：Δ = max_s |V_{k+1}(s) - V_k(s)| < θ（如1e-4）
...
class DPinython:
    ...
    def __init__(self, n_states, n_actions, gamma=0.99, theta=1e-4):
        ...
        # 初始化模型（示例：网格世界）
        self.P = {}  # P[s][a] = [(prob, s_next, reward), ...]
        self.V = [0.0] * n_states
...
def visualize_dp_results(V, policy, title="DP Results"):
    ...
    plt.tight_layout()
    plt.savefig(f'dp_{title}_visualization.png', dpi=300)
    plt.show()
```
...
1. 贝尔曼方程（策略π）：$$ V^\pi(s) = \sum_a \pi(a|s) \sum_{s',r} P(s',r|s,a)[r + \gamma V^\pi(s')] $$
2. 贝尔曼最优方程：$$ V^*(s) = \max_a \sum_{s',r} P(s',r|s,a)[r + \gamma V^*(s')] $$
3. 策略评估更新：$$ V_{k+1}(s) = \sum_a \pi(a|s) \sum_{s',r} P(s',r|s,a)[r + \gamma V_k(s')] $$
4. 价值迭代更新：$$ V(s) \leftarrow \max_a \sum_{s',r} P(s',r|s,a)[r + \gamma V(s')] $$
