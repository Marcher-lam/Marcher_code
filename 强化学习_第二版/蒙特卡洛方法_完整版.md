...
- 回报（Return）：G_t = r_{t+1} + γ r_{t+2} + ... + γ^{T-t} r_T
...
2. **episode采样循环**：生成完整的episode轨迹
   ...
   - 计算每个时刻的回报G_t = sum_{k=t}^{T} γ^{k-t} r_k
...
- **回报（Return）G_t**：从时刻t开始到episode结束的折扣累计奖励，G_t = r_{t+1} + γ r_{t+2} + ... + γ^{T-t} r_T
...
1. **预测问题**：估计给定策略π的价值函数 $V^\pi(s)$ 或 $Q^\pi(s,a)$
   ...
   其中 $G_t = \sum_{k=0}^{T-t} \gamma^k R_{t+k+1}$，T是episode终止时刻。
...
**预测问题**：最小化价值估计的均方误差
$$ L(V) = \mathbb{E}_\pi \left[ (G_t - V(S_t))^2 \right] $$
...
其中 $G_t^{(i)}(s)$ 是第i个episode中状态s在时刻t的回报。

**Step 2：增量式更新**

为了避免存储所有回报，使用增量式更新（类似随机梯度下降）：
$$ V(s_t) \leftarrow V(s_t) + \alpha \left[ G_t - V(s_t) \right] $$
这里α是学习率，等价于给每个回报的权重为 $\alpha(1-\alpha)^{k-1}$（指数衰减）。
...
修正后的回报为 $\rho_{t:T-1} G_t$，用于更新Q(s_t,a_t)。
...
重复（每个episode）：
    ...
    计算所有时刻的回报 G_t = sum_{k=0}^{T-t-1} γ^k R_{t+k+1}
    对轨迹中每个首次出现的状态S_t：
    ...
    def train(self, env, num_episodes=50000):
        ...
        for episode in range(num_episodes):
            ...
            if (episode + 1) % 10000 == 0:
                ...
                print(f"Episode {episode+1}/{num_episodes}, Avg Test Score: {np.mean(test_scores):.2f}")
...
if __name__ == "__main__":
    ...
    print(f"环境: {env.spec.id}")
    print(f"状态空间: {env.observation_space} (玩家点数, 庄家明牌, 可用Ace)")
    print(f"动作空间: {env.action_space} (0=停牌, 1=要牌)")
    ...
    print(f"平均得分: {np.mean(test_scores):.2f} ± {np.std(test_scores):.2f}")
    print(f"赢率: {sum(1 for s in test_scores if s > 0)/len(test_scores):.2%}")
    ...
    def train(self, env, num_episodes=1000, max_steps=100):
        ...
        for episode in range(num_episodes):
            ...
            if (episode + 1) % 100 == 0:
                ...
                print(f"Episode {episode+1}/{num_episodes}, Avg Score: {avg:.2f}")
...
def evaluate_mc_statistically(agent, env, num_runs=10, num_episodes=1000):
    ...
    for run in range(num_runs):
        ...
        print(f"Run {run+1}/{num_runs} done, Avg Score: {np.mean(scores):.2f}")
    
    # 统计汇总
    all_scores = np.array(all_scores)
    print(f"\n平均性能: {np.mean(all_scores):.2f} ± {np.std(all_scores):.2f}")
    print(f"最小性能: {np.min(all_scores):.2f}")
    print(f"最大性能: {np.max(all_scores):.2f}")
...
✓ **优化目标**：最大化期望累计折扣回报 $J(\pi) = \mathbb{E}_\pi[G_0]$
...
3. 重要度采样比：$$ \rho_{t:T-1} = \prod_{k=t}^{T-1} \frac{\pi(a_k|s_k)}{b(a_k|s_k)} $$
4. 加权重要度采样更新：$$ Q(s_t,a_t) \leftarrow Q(s_t,a_t) + \frac{\rho_t}{C(s_t,a_t)} [G_t - Q(s_t,a_t)] $$
