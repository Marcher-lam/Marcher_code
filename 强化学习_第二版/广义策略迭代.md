...
**一句话定义**：广义广义策略迭代（Dynamic Programming, DP）利用完整的环境模型（状态转移概率P和奖励函数R），通过贝尔曼方程的迭代计算，高效求解最优价值函数V*和最优策略π*。

**直觉类比**：想象你在规划从家到公司的最短路线，手里有一张完整的地图（环境模型），上面标明了每条路的行车时间（转移概率）和拥堵情况（奖励）。广义广义策略迭代就是利用这张完整地图，从公司倒推（或正向计算），算出每个路口到公司的最短时间，最终得到全局最优路线。DP不需要实际开车试错，因为地图已经告诉你所有信息。

**历史背景**：广义广义策略迭代由理查德·贝尔曼（Richard Bellman）于1950年代提出，最初用于优化决策问题。20世纪80年代被引入强化学习领域，成为与蒙特卡洛、时序差分并列的三大基础方法。Sutton和Barto的教材系统梳理了DP在RL中的应用，奠定了其理论基础。
...
广义广义策略迭代的核心思想是：利用已知的环境模型（转移概率P和奖励R），通过贝尔曼方程的迭代更新，逐步收敛到最优价值函数V*和最优策略π*。DP是**bootstrapping（自举）**的典型代表：用当前估计的价值函数来计算下一步的更新目标，不断迭代直到收敛到贝尔曼方程的固定点。
...
- **广义策略迭代**：交替进行策略评估和策略改进，直到收敛到π*
- **动态规划**：将策略评估压缩到一步，直接迭代求解V*
- **广义广义策略迭代（GPI）**：任何让策略评估和策略改进交互的通用框架

### 2.4 几何/直观解释

广义广义策略迭代可以看作是在状态空间中“倒推”或“正向传播”价值：从终止状态开始，利用转移概率向后计算每个状态的价值，直到所有状态的价值都更新完毕。这就像在地图上从终点倒推每个路口的最短距离，最终得到全局最优路线。
...
给定马尔可夫决策过程 $M = \langle S, A, P, R, \gamma \rangle$，广义广义策略迭代解决两个核心问题：
...
class DPAgent:
    ...
    def policy_iteration(self):
        ...
        for i in range(100):  # 最多100次广义策略迭代
            # 1. 策略评估
...
2. **理论保证收敛**：在有限MDP中，广义策略迭代和动态规划都保证收敛到最优解
   - 成立条件：γ < 1，状态/动作空间有限
...
| 维度 | 广义广义策略迭代（DP） | 蒙特卡洛（MC） | 时序差分（TD） |
|------|----------------|----------------|--------------|
| 模型需求 | 需要完整模型 | 无模型 | 无模型 |
| 样本效率 | 最高（零采样） | 低 | 中 |
...
"""
广义广义策略迭代 手工实现
仅依赖Python基础库，实现广义策略迭代和动态规划
"""

import random

class DPinython:
    """广义广义策略迭代从零实现（广义策略迭代+动态规划）"""
...
| 方法 | 收敛速度 | 计算时间 | 代码复杂度 |
|------|---------|----------|------------|
| 调库实现（完整DP） | 广义策略迭代2轮，动态规划273次 | 快（优化） | 中等 |
| 手工实现（简化DP） | 广义策略迭代2轮，动态规划~300次 | 中等 | 低 |

**分析**：
- 手工实现与调库结果一致，验证了DP的正确性
- 广义策略迭代通常比动态规划收敛更快（更少的外层迭代）
- 手工实现更直观，适合理解算法原理
...
**错误2：混淆广义策略迭代和动态规划**
- 现象：实现错误，结果不符合预期
- 原因：不清楚两者的区别（广义策略迭代交替评估/改进，动态规划直接迭代V*）
...
- **后续算法**：广义广义策略迭代（GPI）、Dyna架构（结合模型学习和规划）
...
1. **广义广义策略迭代（GPI）**：统一DP、MC、TD的框架
2. **Dyna架构**：结合模型学习和规划
3. **实时广义广义策略迭代（RTDP）**：处理大规模MDP
