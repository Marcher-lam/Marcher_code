...
AdaBoost 的核心贡献在于证明了"弱学习器可以通过自适应加权组合提升为强学习器"，并在理论上给出了训练误差的指数衰减上界。其巧妙之处在于：每一轮自动放大被错误分类样本的权重，迫使后续弱分类器专注于"补短板"，而最终的加权投票实现了集体智慧的汇聚。

AdaBoost 的关键优势是实现简单、无需调参即可获得不错效果，且自带特征选择功能（弱分类器的权重反映了其重要性）。它最适合二分类场景，尤其是人脸检测（Viola-Jones 框架）等需要快速级联判别的应用。但对噪声和异常值敏感，因为异常样本会被反复赋予极高权重。

在知识体系中，AdaBoost 是 Boosting 家族的起点，它和本库中的 CART 决策树结合后，演化为更强大的 GBDT（用负梯度替代指数损失），进而发展出 XGBoost、LightGBM 等工业级算法。理解 AdaBoost 的前向分步优化视角，有助于从统一框架理解所有 Boosting 方法。

工业实践中，纯 AdaBoost 已较少单独使用，但它的核心思想——关注困难样本、加权投票——深刻影响了 GBDT 的梯度提升和 DNN 中难样本挖掘（Hard Example Mining）策略。
