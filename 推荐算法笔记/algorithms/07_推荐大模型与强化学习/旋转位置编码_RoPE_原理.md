...
# 十、RoPE 与 ALiBi / 可学习位置编码的深度对比

## 10.1 三种位置编码方案的原理对比

| 维度 | RoPE（旋转位置编码） | ALiBi（Attention with Linear Biases） | 可学习位置编码（Learned PE） |
|------|---------------------|--------------------------------------|---------------------------|
| 核心思想 | 旋转 Q/K 向量编码相对位置 | 在注意力分数上加线性偏置 | 为每个位置学习一个嵌入向量 |
| 位置信息类型 | 相对位置 | 相对位置 | 绝对位置 |
| 参数量 | 0（无额外参数） | 0（无额外参数） | $L_{max} \times d$ |
| 数学形式 | $q_m^T R_{m-n} k_n$ | $q_i^T k_j + m \cdot (-(i-j))$ | $E_m + W_q x_m$ |
| 长度外推 | 强（需 NTK/YaRN 缩放） | 极强（天然支持任意长度） | 差（固定最大长度） |
| 计算开销 | 中（复数乘法） | 低（加法偏置） | 低（向量加法） |
| 代表模型 | LLaMA, Qwen, Mistral | BLOOM, MPT, Baichuan | BERT, GPT-2, T5 |

## 10.2 数学本质差异

**RoPE** 通过乘法将位置信息融入注意力分数：

$$\text{Attn}(m, n) = q^T R_{m-n} k = \sum_{i} q_i k_i \cos((m-n)\theta_i) + \text{cross terms}$$

注意力分数自然包含相对位置 $(m-n)$，且具有远程衰减性。

**ALiBi** 通过加法偏置直接注入相对位置：

$$\text{Attn}(m, n) = q_m^T k_n + m \cdot (j - i)$$

其中 $m$ 是每个注意力头独立学习的斜率（通常为 $2^{-8/n_h}, 2^{-7/n_h}, \ldots$）。ALiBi 不修改 Q/K 向量，仅在注意力分数上加线性衰减。

**可学习 PE** 通过加法注入绝对位置：

$$h_m = W_q(x_m + p_m), \quad p_m \in \mathbb{R}^d \text{ 为可学习参数}$$

位置信息通过嵌入向量直接注入，但经过线性变换后位置信息容易被破坏。

## 10.3 长度外推能力对比

| 方法 | 训练 4K → 推理 8K | 训练 4K → 推理 32K | 训练 4K → 推理 128K |
|------|-------------------|-------------------|---------------------|
| 可学习 PE | 效果急剧下降 | 几乎无法使用 | 完全失效 |
| ALiBi | 无需任何调整，效果稳定 | 无需调整，略有退化 | 无需调整，退化可控 |
| RoPE（无缩放） | 效果明显下降 | 几乎无法使用 | 完全失效 |
| RoPE + NTK Scaling | 效果稳定 | 效果较好 | 需要微调 |
| RoPE + YaRN | 效果稳定 | 效果稳定 | 效果较好 |

## 10.4 优缺点分析

### RoPE 的优势
1. **数学优雅**：注意力分数天然编码相对位置，无需额外偏置项
2. **远程衰减**：邻近 token 关联更强，符合语言先验
3. **与 Flash Attention 兼容**：RoPE 作用在 Q/K 投影后，不改变注意力计算流程
4. **社区生态成熟**：LLaMA 系列开源后，RoPE 已成为事实标准

### RoPE 的劣势
1. **原生外推能力有限**：超出训练长度后效果下降，需要 NTK/YaRN 等辅助技术
2. **复数运算开销**：虽然可优化，但比 ALiBi 的加法偏置计算成本更高
3. **频率选择敏感**：base 值（通常 10000）影响不同粒度的位置区分能力

### ALiBi 的优势
1. **零成本长度外推**：无需任何修改即可处理任意长度序列
2. **无额外参数**：仅通过线性偏置注入位置，不增加模型参数
3. **实现简单**：在注意力分数上加一行偏置即可

### ALiBi 的劣势
1. **线性衰减假设过强**：假设注意力随距离线性衰减，可能不适用于所有任务
2. **无法精确编码相对位置**：偏置是固定的线性函数，不像 RoPE 能通过不同频率捕捉多粒度位置
3. **社区采用率下降**：随着 RoPE + NTK/YaRN 方案成熟，ALiBi 的外推优势被追平

## 10.5 何时选择 RoPE

| 场景 | 推荐方案 | 原因 |
|------|---------|------|
| 通用 LLM（7B+） | RoPE + YaRN | 数学优雅，社区生态成熟 |
| 超长文本（128K+） | RoPE + YaRN 或 ALiBi | 两者均可，RoPE+YaRN 效果更优 |
| 资源受限推理 | ALiBi | 无需复数运算，推理更快 |
| 推荐系统序列建模 | RoPE 或可学习 PE | 序列通常 <512，长度外推非关键需求 |
| 多模态模型 | RoPE | 可自然扩展到 2D/3D 位置编码 |
| BERT 类双向模型 | 可学习 PE 或 RoPE | 双向注意力下 RoPE 的相对位置优势减弱 |
