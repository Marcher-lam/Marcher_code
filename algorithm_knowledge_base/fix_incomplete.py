...
def fix_incomplete_file(filepath):
    ...
    # 检查有多少章
    chapter_matches = re.findall(r'^## (\d+)\.', content, re.MULTILINE)
    if not chapter_matches:
        return "未找到章节标记"
    
    last_chapter = max(int(c) for c in chapter_matches)
    
    if last_chapter < 7:
        return f"只有{last_chapter}章，内容不足"
    ...
    # 添加缺失章节
    content += CHAPTERS_8_TO_14
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    
    return f"已添加第8-14章（原第{last_chapter}章）"

def main():
    alg_dir = "/Users/marcher/Desktop/Marcher_code/algorithm_knowledge_base/algorithms"
    
    # 需要修复的文件列表（只有7章）
    files_to_fix = [
        "CenterNet.md", "DenseNet.md", "Inception.md", "PSPNet.md",
        "Mask_R-CNN.md", "ResNeXt.md", "SENet.md", "SSD.md",
        "MobileNet.md", "DeepSpeech.md", "DeiT.md", "Informer.md",
        "EfficientNet_v2.md", "Swin_Transformer.md"
    ]
    
    results = []
    for fname in files_to_fix:
        fpath = os.path.join(alg_dir, fname)
        if os.path.exists(fpath):
            result = fix_incomplete_file(fpath)
