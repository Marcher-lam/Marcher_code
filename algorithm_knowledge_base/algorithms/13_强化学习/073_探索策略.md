...
探索策略是强化学习领域中的一种重要算法/方法。它通过数据驱动的方式，从观测数据中学习模式并建立预测或决策模型，实现对未知数据的泛化。

探索策略在机器学习发展历程中具有重要地位，理论基础不断完善，应用范围持续拓展。理解探索策略对于掌握更高级算法和技术具有重要意义。
...
探索策略的关键在于平衡模型复杂度与泛化能力，通过合理的正则化和模型选择取得良好效果。
...
plt.plot(losses); plt.xlabel('Iteration'); plt.ylabel('Loss'); plt.title('探索策略 Training'); plt.show()
```
...
### 进阶方向
掌握探索策略后，可学习相关变体和进阶方法。


## 13. 练习题与思考题

**练习1：** 简述探索策略核心思想及适用场景。
<details><summary>答案</summary>探索策略通过数据驱动学习映射关系，适用于强化学习中的多种任务。</details>

**练习2：** 写出探索策略的损失函数并推导梯度。
<details><summary>答案</summary>
$$L(\theta) = \frac{1}{N} \sum_{i} \ell(y_i, f(x_i; \theta)), \quad \nabla_\theta L = \frac{1}{N} \sum_{i} \nabla_\theta \ell$$
</details>

**练习3：** 用Python实现探索策略核心逻辑并测试。
<details><summary>答案</summary>参考第8章手工代码实现。</details>

**思考题：** 探索策略在什么情况下失效？数据量少时如何处理？如何与其他方法结合？
...
### 推荐资源
探索策略原始论文、GitHub实现、Coursera/Stanford课程
